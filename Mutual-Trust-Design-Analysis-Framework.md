...
| --------------------------- | ----------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Ký tự bullet                | `•` (U+2022)                                                            | Luôn font **SymbolMT** — artefact Word export, không phải chủ đích thiết kế (đã ghi ở B.1)                                                                                                                                                                |
...
| Khoảng cách marker → text   | **18pt cố định** (≈ 24px web)                                           | Đo giống nhau tuyệt đối ở mọi file dù cỡ chữ khác nhau (9.96–12pt) và dù là bullet (`•`) hay số (`1.`) — đây là tab-stop mặc định của Word (0.25in), **không** tỷ lệ theo font-size                                                                       |
...
| Line-height trong list item | ≈ **1.2× cỡ chữ**, giống thân bài                                       | Không có leading riêng cho list (xem B.2)                                                                                                                                                                                                                 |
...
--list-indent:        24px;               /* text-indent cho hanging indent, = marker-gap */
```

```css
/* Base: kiểu (a) Flush — marker canh lề thân bài, text +24px, hanging indent khi wrap */
ul.mt-list,
ol.mt-list {
  list-style: none;
  padding-left: 0;
...
1. **Hero/Header** — 2 biến thể layout PDF: (a) title overlay trên ảnh — Market Review; (b) logo overlay + title tách khối nội dung — Outlook/Insight. Logo: URL A.5 (`logo-m.svg` hoặc `MT-Logo.svg` theo PDF).
...
10. **List (bullet & numbered)** — marker màu ink (`--color-ink`, không Ochre); gap marker→text cố định 24px (không theo cỡ chữ); hanging indent cho dòng wrap; numbered dùng số body font + dấu `.` (không font riêng); base variant = Flush, có 2 biến thể Outdent/Inset theo file nguồn — chi tiết token & CSS ở B.9.
...
5. **Calibrate geometry rồi typography** theo C.6.4–C.6.5 (content width, size, line-height, spacing, placement từ PDF).
6. Giữ toàn bộ copy PDF — không omit / invent / duplicate.
7. Semantic / one-off colours và link treatment theo PDF khi có (C.6.7).
8. List/bullet theo B.9 + C.6.6 — không dùng browser default.
9. **Không tái tạo pagination của PDF trên web.** Nội dung chuyển thành continuous document flow. Page boundary của PDF chỉ dùng làm checkpoint trong visual QA để phát hiện cumulative spacing/typography drift (C.6.8–C.6.9).
10. Chạy Visual Comparison Loop (C.6.8) — tối đa 3 vòng correction nếu còn khác biệt đáng kể; **desktop fidelity đạt trước** rồi mới responsive (C.6.10).
11. Chỉ đánh dấu hoàn thành khi đạt Definition of Done (C.6.11).
12. **Chưa deploy** cho đến khi được yêu cầu.
...
Phải đối chiếu: marker type/size/position, gap marker→text, hanging indent, wrapped-line alignment, item spacing, khoảng trước/sau list.

Dùng pattern Flush / Outdent / Inset đã phân tích tại B.9.

### C.6.7 Link và màu

Không tự động áp style link web generic.

Link phải đối chiếu PDF về: colour, underline, font weight. Hover chỉ là enhancement cho web.
